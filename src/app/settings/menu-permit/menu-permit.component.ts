...
import { SettingsService, MenuPermitRow, UserRow } from '../../core/services/settings.service';
...
@Component({
  selector: 'app-menu-permit',
  standalone: false,
  templateUrl: './menu-permit.component.html',
  styleUrl: '../../developer/developer-page.scss',
})
export class MenuPermitComponent implements OnInit {
  private settings = inject(SettingsService);
  private modules = inject(ModuleService);
  private notify = inject(NotificationService);

  users: UserRow[] = [];
  permits: MenuPermitRow[] = [];
  selectedUserId = 0;
  loading = false;
  saving = false;
  ...
  readonly columnDefs: ColDef[] = [
    { headerName: '#', field: 'MenuID', maxWidth: 90, flex: 0 },
    { headerName: 'Module', field: 'ModuleName', maxWidth: 200 },
    {
      headerName: 'Code',
      field: 'MenuCode',
      maxWidth: 160,
    ...
    { headerName: 'Menu', field: 'MenuName' },
    {
      headerName: 'Visible',
      field: 'ISVisible',
      maxWidth: 140,
  ...
  onUserChange(): void {
    ...
    this.loading = true;
    this.settings.getMenuPermit(this.selectedUserId).subscribe({
  ...
  save(): void {
    ...
    this.saving = true;
    const payload = this.permits.map((p) => ({ menuId: p.MenuID, moduleId: p.ModuleID, isVisible: !!p.ISVisible }));
    this.settings.saveMenuPermit(this.selectedUserId, payload).subscribe({
